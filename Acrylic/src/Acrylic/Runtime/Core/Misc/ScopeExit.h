...
#define AC_PRIVATE_SCOPE_EXIT_JOIN(A, B) AC_PRIVATE_SCOPE_EXIT_JOIN_INNER(A, B)
#define AC_PRIVATE_SCOPE_EXIT_JOIN_INNER(A, B) A##B
 ...
 *    }
 */
#define ON_SCOPE_EXIT const auto AC_PRIVATE_SCOPE_EXIT_JOIN(ScopeGuard_, __LINE__) = ScopeExitSupport::FScopeGuardSyntaxSupport() + [&]()
} // namespace Acrylic